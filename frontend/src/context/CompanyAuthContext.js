...
export const CompanyAuthProvider = ({children}) => {
    ...
    let [companyAuthTokens, setCompanyAuthTokens] = useState(()=> localStorage.getItem('companyAuthTokens') ? JSON.parse(localStorage.getItem('companyAuthTokens')) : null)
    // If the local storage contains authTokens, then decode the token, else set that to null
    let [company, setCompany] = useState(()=> localStorage.getItem('companyAuthTokens') ? jwt_decode(localStorage.getItem('companyAuthTokens')) : null)
    ...
    let loginCompany = async (e )=> {
        ...
        if(response.status === 200){
            ...
            if(jwt_decode(data.access)['group'] === 'Mentor' || jwt_decode(data.access)['group'] === 'None'){
    ...
    // To register a user
    let registerCompany = async (e) => {
        e.preventDefault()

        // Make a post request to the api with the user's credentials.
        let response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/register-company/`, {
            method:'POST',
            headers:{
                'Content-Type':'application/json'
            },
            // 'e.target' is the form, '.username' gets the username field and '.password' gets the password field from wherever it is called (LoginPage.js here)
            body:JSON.stringify({'company':e.target.company.value, 'website':e.target.website.value, 'email':e.target.email.value, 'password':e.target.password.value, 'confirmPassword':e.target.confirmPassword.value})
        })
        // Get the access and refresh tokens
        let data = await response.json()
