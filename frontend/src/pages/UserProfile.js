...
const UserProfile = ({match}) => {
    ...
    useEffect(()=> {
        ...
        let getUserInfo = async() =>{
            let response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/get-info/`, {
                method:'GET',
                headers:{
    ...
    let addInfo = async (e )=> {
        ...
        let url = `${process.env.REACT_APP_BACKEND_URL}/api/add-info/`;
    ...
    return (
        <div>
            ...
            <div>
                {userInfo.map(info => (
                    <div key={info.id}>
                        <p> Name: {info.name} </p>
                        <p> Bio: {info.bio} </p>
                        <p> Image: <img src={info.image} alt='userPhoto'/> </p> 
                        {mentor ?
                        <div>
                            <p> Timings: {info.startTime} - {info.endTime} {info.timeZone} </p>
                        </div>
                        : (null)}
                    </div>
                ))}
            ...
            <form onSubmit={addInfo}>
                <input type="file" accept="image/x-png,image/jpeg,image/jpg" name="image" placeholder="User image"/> <br/>            
                <input type='text' name='name' placeholder='name' /> <br/> 
                <input type='text' name='bio' placeholder='bio' /> <br/>
                <input type="time" name='startTime' hidden={!mentor} defaultValue=''/> 
                <input type="time" name='endTime' hidden={!mentor} defaultValue=''/> <br/>
                <input type="text" name="timeZone" hidden={!mentor} placeholder='Time Zone (EST/IST)' defaultValue=''/> <br/>    
